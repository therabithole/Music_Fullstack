...
const url = "mongodb://localhost/spotify";
...
// const multipleMedia = upload.array("picture", 12);

const uploadBuffer = (req, res, next) => {
  const upload = store.single("buffer");
  upload(req, res, function (err) {
    if (err instanceof multer.MulterError) {
      return res.status(400).send("Buffer File too large");
    } else if (err) {
      // check if our filetype error occurred
      if (err === "filetype")
        return res.status(400).send("Buffer Image files only");
